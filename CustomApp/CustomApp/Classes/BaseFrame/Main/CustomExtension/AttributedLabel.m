...
- (void)drawRect:(CGRect)rect{
    ...
    textLayer.frame = CGRectMake(20, 0, self.frame.size.width, self.frame.size.height);
...
- (void)setFont:(UIFont *)font fromIndex:(NSInteger)location length:(NSInteger)length{
    ...
    [_attString addAttribute:(NSString *)kCTFontAttributeName
                        value:(id)CFBridgingRelease(CTFontCreateWithName((CFStringRef)font.fontName,font.pointSize*2,NULL))
                        range:NSMakeRange(location, length)];
}
